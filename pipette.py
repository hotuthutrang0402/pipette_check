...
def export_pdf(df: pd.DataFrame, metadata=None) -> BytesIO:
    ...
    if metadata:
        ...
        elements.append(Paragraph(f"Địa điểm thực hiện: {metadata['dia_diem']}", styles["Normal"]))
...
def render_downloads(df: pd.DataFrame):
    today_str = datetime.today().strftime("%Y%m%d")  # ví dụ: 20260429
    # ===== EXCEL =====
    excel_buffer = export_excel(df)
    st.download_button(
        "📥 Tải xuống file Excel",
        excel_buffer,
        file_name=f"{today_str}_pipette_qc.xlsx",
    ...
    st.download_button(
        "📄 Tải xuống file PDF",
        pdf_buffer,
        file_name=f"{today_str}_pipette_qc.pdf",
